...
<!--         <xsl:value-of select="concat('//', //dri:metadata[@qualifier='serverName'], ':',//dri:metadata[@qualifier='serverPort'])" /> -->
    ...
    <xsl:template match="dim:dim" mode="itemSummaryView-DIM">
	    ...
	    <xsl:if test="(//dim:field[@element='type'] = 'bookChapter') or (//dim:field[@element='type'] = 'bookPart')">
	       <xsl:call-template name="itemSummaryView-DIM-references"/> 
	   </xsl:if>
	</div>
    </xsl:template>
...
<!-- 		<div data-badge-popover="right" data-badge-type="2" data-doi="${}" data-hide-no-mentions="true" class="altmetric-embed">
	...
	<xsl:if test="not(//dim:field[@element='relation' and @qualifier='references'])">
    ...
    <xsl:template name="itemSummaryView-DIM-references">
        <xsl:if test="//dim:field[@element='relation' and @qualifier='references' and descendant::text()]">
	  <div class="row litref">
	     <div class="col-sm-12">
		<a class="refs" href="#" onclick="$('#litref').toggle();">
                <i18n:text>xmlui.dri2xhtml.item.litref</i18n:text>
		</a>
		
		<div id="litref"> 
		<xsl:value-of select="//dim:field[@element='relation' and @qualifier='references']" disable-output-escaping="yes" />
		</div>  
	      </div>
          </div>
	</xsl:if>
...
<!--		<xsl:variable name="metsData" select="document($parentMetadataUrl)/mets:METS/mets:dmdSec/mets:mdWrap/mets:xmlData/dim:dim"/>		-->
   ...
   <xsl:variable name="metsData" select="document($parentMetadataUrl)//dim:dim"/>
	...
	<span class="chapter">
		<xsl:choose>
			<xsl:when test="$metsData/dim:field[@element='type' and not(@qualifier)] = 'monograph'">
				<i18n:text>xmlui.item.chapter</i18n:text>
			</xsl:when>
			<xsl:otherwise>
				<i18n:text>xmlui.item.part</i18n:text>
			</xsl:otherwise>
		</xsl:choose>
	<xsl:value-of select="concat(' ', substring-after(/mets:METS/@ID, '.'))"/>
	</span>
	...
	<xsl:if test="//dim:field[@element='bibliographicCitation'][@qualifier='firstpage']">
		...
		<xsl:value-of select="concat(' ', //dim:field[@element='bibliographicCitation'][@qualifier='firstpage'], '-', //dim:field[@element='bibliographicCitation'][@qualifier='lastpage'], ')')"/>  
	</xsl:if>
	<xsl:text> </xsl:text>
	<i18n:text>xmlui.item.chapter.of</i18n:text>
...
<div class="row">
  ...
  <div class="col-xs-hidden col-sm-hidden col-md-4 col-lg-4">
   ...
   <div class="col-xs-12 col-sm-12 col-md-8 col-lg-8">
	...
	<xsl:if test="$metsData/dim:field[@element='title' and @qualifier='translated']">
                                <br />
                                <xsl:copy-of select="$metsData/dim:field[@element='title' and @qualifier='translated'][1]/node()"/>
        </xsl:if>
</a></h3>
     <h4>
	<xsl:if test="$metsData/dim:field[@element='title' and @qualifier='alternative']">
		...
		<xsl:if test="$metsData/dim:field[@element='title' and @qualifier='alternativetranslated']">
                                <br />
                                <xsl:copy-of select="$metsData/dim:field[@element='title' and @qualifier='alternativetranslated'][1]/node()"/>
	        </xsl:if>
        </xsl:if>
     </h4>
     <div class="itemView-authors">
	...
	<i18n:text>xmlui.dri2xhtml.item.editor</i18n:text>
	   ...
	   <xsl:if test="//dim:field[@element='relation'][@qualifier='otherparts']">
                <li><a data-target="#related" data-toggle="tab"><i18n:text>xmlui.dri2xhtml.METS-1.0.item-related.items</i18n:text></a></li>
            </xsl:if>
	    <xsl:if test="//dim:field[@element='description' and @qualifier='authors']">
		<li>
			<a data-target="#authors" data-toggle="tab"><i18n:text>xmlui.dri2xhtml.METS-1.0.item-aboutauthors</i18n:text></a>
		</li>
	    </xsl:if> 
	    ...
	    <xsl:if test="//dim:field[@qualifier='references']">
		<li><a data-target="#references" data-toggle="tab"><i18n:text>xmlui.dri2xhtml.METS-1.0.item-references</i18n:text></a></li>
	    </xsl:if>
		...
		<div class="tab-pane" id="authors">	
			<xsl:value-of select="//dim:field[@element='description' and @qualifier='authors']" disable-output-escaping="yes"/>
		</div>
	    </xsl:if>
	    <div id="references" class="tab-pane"> 
			<xsl:value-of select="//dim:field[@qualifier='references']" disable-output-escaping="yes" />
	    </div>
    ...
    <!-- show table of contents if parts of the publication have DOI -->
		...
		<h3>
			<xsl:choose>
				<xsl:when test="//dim:field[@element='type' and not(@qualifier)] = 'monograph'">
					<i18n:text>xmlui.item.chapter.list</i18n:text>
				</xsl:when>
				<xsl:otherwise>
					<!-- If MKWI complete articles -->
					<xsl:choose>
						<xsl:when test="//dim:field[@qualifier='articlenumber'] = '8100798'">
							<i18n:text>xmlui.item.complete.article.list</i18n:text>
						</xsl:when>
						<xsl:otherwise>
							<i18n:text>xmlui.item.article.list</i18n:text>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:otherwise>
			</xsl:choose>
   ...
   <xsl:variable name="metsData" select="document($childMetadataUrl)//dim:dim"/>
				...
				<xsl:if test="$metsData/dim:field[@element='bibliographicCitation'][@qualifier='firstpage']">
