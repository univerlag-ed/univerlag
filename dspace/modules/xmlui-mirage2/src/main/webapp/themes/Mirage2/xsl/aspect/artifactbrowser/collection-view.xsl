...
<xsl:stylesheet
    ...
    <xsl:template match="dim:dim" mode="collectionDetailView-DIM">
        <xsl:if test="string-length(dim:field[@element='description'][not(@qualifier)])&gt;0">
            <p class="intro-text">
                <!-- make intro text multilingual -->
                <i18n:text><xsl:value-of select="dim:field[@element='description'][not(@qualifier)]"/></i18n:text>

            </p>
        </xsl:if>

        <xsl:if test="string-length(dim:field[@element='description'][@qualifier='tableofcontents'])&gt;0">f
