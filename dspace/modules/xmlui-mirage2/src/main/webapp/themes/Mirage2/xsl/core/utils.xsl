...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template name="standardAttributes">
        ...
        <!-- make intern doi field readonly. Should be done in preprocess -->
        <xsl:if test="@id = 'aspect.submission.StepTransformer.field.dc_intern_doi'" >
            <xsl:attribute name="readonly"><xsl:text> </xsl:text></xsl:attribute>
        </xsl:if>
        <xsl:attribute name="class">
